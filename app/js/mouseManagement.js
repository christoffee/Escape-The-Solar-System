    ...
    if(screenDepth >= 0.3 && gameData.planet[targetPlanetLowercase].terrafomed){
      ...
      for(var obj in planets){
        ...
        if(currentMousePos.y > y - (unit*size) && currentMousePos.y < y + (unit*size)  && currentMousePos.x > x +((unit*size)*0.5)  && currentMousePos.x < x + ((unit*size)*2.5) ){
          ...
          context.rect(0, 0,unit*1000,unit*1200);
          context.closePath();
          context.fill(); 
          context.restore();
  ...
  function drawBuildMenu () {
    ...
    if(drawMenu){
      var occupied = planetSegments[locationElement][1];
      context.save();
      context.beginPath();
      context.translate(currentMousePos.x,currentMousePos.y);
      ...
      context.font="1em Arial";
      var locationText = "Location " + locationElement;
  ...
  function mouseClick (e) {
    ...
    if(e.x >= canvasWidth/30 && e.x <= canvasWidth/30 + canvasWidth/7  && e.y >= canvasHeight -(canvasHeight/10) && e.y <= (canvasHeight -(canvasHeight/10))+ canvasHeight/15){
      zoomIn = false;
    }
